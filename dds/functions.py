...
# 自作関数, 結果の収納
def dds_result(table):    # C0 D1 H2 S3 NT4, N0 E1 S2 W3,  table.contents.resTable[suit][declarer]
    result = [[0 for i in range(4)] for j in range(5)]
    for declarer in range(4):
        for suit in range(5):
            if suit == 4:
                result[suit][declarer] = table.contents.resTable[suit][declarer]
            else:
                result[3-suit][declarer] = table.contents.resTable[suit][declarer]
    return result
